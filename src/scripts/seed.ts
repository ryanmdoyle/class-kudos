...
export default defineScript(async ({ env }) => {
  ...
  if (!isSupabaseAdminConfigured()) {
    console.warn(
      "⚠️  SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY are not set.\n" +
        "    Seeding a LOCAL-ONLY teacher row with supabaseUserId = null.\n" +
  ...
  console.log(
    `✅ Teacher ${email} (${provisioned.created ? "created" : "updated"}), ` +
      `supabaseUserId=${provisioned.supabaseUserId ?? "null"}`,
  );
  ...
  for (const [firstName, lastName] of STUDENTS) {
    ...
    await db
      ...
      .values({
        ...
        // username. They authenticate purely through classCodes.
        supabaseUserId: null,
        username: null,
        email: null,
        firstName,
